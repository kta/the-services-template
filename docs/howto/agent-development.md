...
## 単一ソースの指示
- **`AGENTS.md`**（ルート）= 全エージェント共通の指示（[agents.md](https://agents.md/) 標準）。Cursor / Codex はこれを**ネイティブに読む**。
- **`CLAUDE.md`** = `AGENTS.md` への**シンボリックリンク**（実体は 1 つ。Claude 固有メモは AGENTS.md の「エージェント固有メモ」節に置く）。
- **`.github/copilot-instructions.md`** = Copilot 向けポインタ。
- 重複は持たせない（AGENTS.md が正）。常時ロードは AGENTS.md だけに抑え、詳細規約は**タスク別ロードガイド**から必要時のみ読む（progressive disclosure — 命令数を増やすほど遵守率は下がる）。

## 完了の定義（最重要）
**`pnpm check`**（lint + typecheck + test、カバレッジゲート込み）が緑＝完了。エージェントはこの pass/fail を頼りに無人で回せる。`make check` でも可。
...
- **toolchain ピン**: `mise.toml`（node 22 / pnpm 10）。`mise install`。
- **hooks**（`lefthook.yml`）: pre-commit に biome、commit-msg に commitlint（Conventional Commits）、pre-push に typecheck+test。「必ず守らせたいこと」はプロンプトでなく hook に置く。
...
3. **TDD**: テスト先行 → 実装 → `pnpm check` 緑。
4. **PR**: Conventional Commits、US-ID 明記、CI（verify = lint+typecheck+unit）必須。e2e は手動トリガ（`workflow_dispatch`）またはローカルでオンデマンド。
5. **依存更新**: `renovate.json`（dev 依存の非メジャーは緑で自動マージ、catalog は人間レビュー）。
