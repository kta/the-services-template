...
## 完了の定義（最重要）
**`pnpm check`**（lint + Knip dependency audit + typecheck + test、カバレッジゲート込み）が緑＝完了。エージェントはこの pass/fail を頼りに無人で回せる。`make check` でも可。
...
- **hooks**（`lefthook.yml`）: pre-commitにBiome+unit、commit-msgにcommitlint、pre-pushにlint+dependency audit+typecheck+coverage test。「必ず守らせたいこと」はプロンプトでなくhookに置く。
...
4. **PR**: Conventional Commits、US-ID 明記、CI（verify = agent compatibility + lint + dependency audit + typecheck + coverage unit）必須。e2e は手動トリガ（`workflow_dispatch`）またはローカルでオンデマンド。
5. **依存更新**: [`dependency-management.md`](./dependency-management.md) に従い、Knipで不要/暗黙依存を検査する。Renovateは安全なdev依存だけ自動化し、catalogとmajorは人間レビュー。
