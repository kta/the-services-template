...
この文書は services/example_service の Tauri v2 shell をローカルで確認し、検証 artifact を作るための手順である。example_service はコピー元の雛形なので、本番 Worker としての deploy や、この dev 認証をそのまま使った配布は行わない。

## まず使うコマンド

| 目的 | コマンド |
| --- | --- |
| Web の SPA + Worker API | make dev/example_service |
| Tauri desktop 開発 | make dev/example_service/tauri（desktop専用） |
| Tauri 静的 frontend bundle | make build/example_service/tauri |
| Tauri CLI 情報 | pnpm --filter @app/example_service tauri info |
| Android emulator / 実機 live development | pnpm --filter @app/example_service exec tauri android dev |
| iOS simulator / 実機 live development | pnpm --filter @app/example_service exec tauri ios dev |
| Rust unit test | cargo test --manifest-path services/example_service/src-tauri/Cargo.toml |

`make dev/example_service/tauri` は Tauri desktop の `tauri dev` を起動する Make target であり、Android/iOS の live development には使わない。通常の Web server と同じ 5173 port を使うため、`make dev/example_service` と同時に起動しない。port collision 時は Vite が別 port へ逃げずに失敗する。

## Web と native の違い

- Web は http://localhost:5173 の同一 Worker へ relative fetch する。
...
example_service の /api/auth/token は AUTH_DEV_GRANT=true と AUTH_DEV_PRIVATE_KEY がローカル設定にあるときだけ credential-less に任意 workspace の token を発行する。本番では両方を設定せず、real login / refresh と tenant policy を実装してから配布する。
...
初回は make init で dev vars、local D1 migration、seed を用意する。
    ...
    make init
    make dev/example_service/tauri

Web だけを確認する場合:

    make dev/example_service

Tauri 用の静的 bundle だけを確認する場合:

    make build/example_service/tauri
...
直接 LAN host へ bind する場合だけ、モバイルの dev コマンドへ `TAURI_DEV_HOST` を渡す。desktop は Make target のままでよい。
    ...
    # Android emulator / 実機
    TAURI_DEV_HOST="$(ipconfig getifaddr en0)" pnpm --filter @app/example_service exec tauri android dev

    # iOS simulator / 実機
    TAURI_DEV_HOST="$(ipconfig getifaddr en0)" pnpm --filter @app/example_service exec tauri ios dev
...
release build では TAURI_EXAMPLE_API_ORIGIN が必須で、HTTPS の origin だけを受け付ける。path、query、fragment、userinfo、末尾 slash は無効である。これは secret ではなく接続先の設定なので Rust build script が binary に埋め込む。
    ...
    TAURI_EXAMPLE_API_ORIGIN=https://example.example.com \
      pnpm --filter @app/example_service exec tauri build \
...
- capability は `windows: ["main"]` に限定し、example_service は `allow-api-request` だけを許可する。`remote` / `webviews`、filesystem、shell、opener、任意 HTTP plugin、任意 command を追加しない（admin は別途 `allow-clear-session` も持つ）。
