...
- **サービス catalog**: SPA サービスの directory/package、`templateKind`、deployable/native、native artifact workflow は root `service-catalog.json` が単一ソース。SPA を持たない notifier/ops は `workerOnlyServices` として同 catalog に明示する。コピー直後に登録して `node scripts/service-catalog.mjs validate-repository` を通し、dev/all・combined test・E2E・production/native CI/Make/workflow の集合を個別に hardcode しない。
