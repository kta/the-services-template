...
- **サービス catalog**: SPA サービスの directory/package、`templateKind`、deployable/native、native artifact workflow は root `service-catalog.json` が単一ソース。コピー直後に登録して `node scripts/service-catalog.mjs validate` を通し、native CI/Make/workflow の集合を個別に hardcode しない。
