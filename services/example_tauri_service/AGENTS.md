# example_tauri_service エージェント指示

このファイルはルート `AGENTS.md` を継承し、このディレクトリで追加適用するサービス固有規約を定める。衝突時はルート規約を優先する。

## 役割

`example_tauri_service` は新しい業務ドメインサービスのコピー元であり、itemを題材に次の標準形を実証する。

- 1つのCloudflare WorkerがReact SPAとHono APIを同一オリジンで配信する。
- サービス専用D1がitemと同期済みorganizationを所有する。
- adminからorganizationをservice bindingで受信し、`ADMIN_TO_EXAMPLE_TAURI_SERVICE_KEY` で認証する。notifierは `x-internal-caller=domain` + `DOMAIN_TO_NOTIFIER_KEY` でservice binding同期呼び出しする。
...
- API契約は `packages/contracts/src/example_service.ts` のZodをWeb雛形と共有し、単一ソースとする。routeはchainを切らず `AppType` を保つ。契約型をこのサービス内へ手書きで複製しない。
...
```sh
pnpm --filter @app/example_tauri_service dev
pnpm --filter @app/example_tauri_service build
pnpm --filter @app/example_tauri_service build:tauri
pnpm --filter @app/example_tauri_service tauri info
pnpm --filter @app/example_tauri_service tauri dev
pnpm --filter @app/example_tauri_service typecheck
pnpm --filter @app/example_tauri_service test       # Worker/integration coverage (各4指標 80%以上)
pnpm --filter @app/example_tauri_service test:web   # React/jsdom coverage (各4指標 60%以上)
pnpm --filter @app/example_tauri_service test:all   # 上記を順に両方実行
pnpm --filter @app/example_tauri_service exec vitest run --config vitest.web.config.ts -t "<test name>"
pnpm --filter @app/example_tauri_service e2e
pnpm --filter @app/example_tauri_service cf-typegen
...
完了前に、対象テスト、`pnpm --filter @app/example_tauri_service typecheck`、必要なe2e、`pnpm run test:traceability`、最後にルート `pnpm check` をgreenにする。secret、deploy、pushはルートの承認規則に従う。
