...
const BASE = 'https://example-tauri-service.test'
const INTERNAL = {
  'content-type': 'application/json',
  'x-internal-key': 'dev-admin-to-example-tauri-service-key-000000000000',
}
...
describe('example_tauri_service API (integration, real D1 in workerd)', () => {
...
describe('example_tauri_service internal org-sync routes (service-binding only)', () => {
  ...
  it('upserts a synced org with the internal key (idempotent) and lists it back', async () => {
    ...
    const list = await SELF.fetch(`${BASE}/api/internal/organizations`, {
      headers: { 'x-internal-key': 'dev-admin-to-example-tauri-service-key-000000000000' },
    })
    expect(list.status).toBe(200)
