...
function signedInAs(organizationId = 'org_acme') {
  sessionStorage.setItem('app.example_tauri_service.auth.token', 'token-123')
  sessionStorage.setItem('app.example_tauri_service.auth.org', organizationId)
}

describe('example Tauri service app', () => {
  afterEach(() => {
    auth.logout()
    vi.unstubAllGlobals()
  })
