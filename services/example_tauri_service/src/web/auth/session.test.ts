...
describe('example Tauri service auth session', () => {
  afterEach(() => {
    invoke.mockReset()
  ...
  it('keeps the existing browser sessionStorage login and bearer behavior', async () => {
    ...
    expect(sessionStorage.getItem('app.example_tauri_service.auth.token')).toBe('browser-token')
    expect(sessionStorage.getItem('app.example_tauri_service.auth.org')).toBe('org_acme')
  ...
  it('keeps native tokens in memory and routes authenticated requests through IPC', async () => {
    ...
    expect(sessionStorage.getItem('app.example_tauri_service.auth.token')).toBeNull()
    expect(sessionStorage.getItem('app.example_tauri_service.auth.org')).toBeNull()
