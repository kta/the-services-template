...
// Tauri materializes the invoke body before the command receives it. Cap the
// serialized envelope before deserializing bounded fields again, so a
// compromised renderer cannot make serde clone an arbitrarily large map.
...
pub fn validate_request(request: &ApiRequest) -> Result<(), String> {
    ...
    // which would otherwise make the raw request and forwarded path differ.
...
impl ApiState {
    pub fn new() -> Self {
        ...
        Self {
            client,
            origin: env!("TAURI_EXAMPLE_TAURI_SERVICE_API_ORIGIN").to_owned(),
...
mod tests {
    use super::{
        decode_ipc_body, filter_response_headers, ipc_payload_within_limit,
        serialize_response_for_ipc, serialized_response_upper_bound, validate_request, ApiRequest,
        ApiResponse, ApiState, BoundedHeaders, BoundedString,
    };
    ...
    fn rejects_unapproved_headers_and_header_control_characters() {
        ...
        let mut duplicate = request("GET", "/api/items");
        duplicate
            .headers
    ...
    fn rejects_get_bodies_and_filters_set_cookie_from_responses() {
        ...
        assert!(!headers.contains_key("set-cookie2"));
        assert!(!headers.contains_key("location"));
    }

    #[test]
    fn rejects_oversized_ipc_fields_before_request_is_sent() {
        let mut request = request("POST", &format!("/api/{}", "a".repeat(2_044)));
        request
            .headers
            .insert("content-type".to_owned(), "x".repeat(8_193));
        request.body = Some("x".repeat(1_048_577));
        assert!(validate_request(&request).is_err());

        request.path = "/api/items".to_owned();
    ...
    #[test]
    fn bounded_ipc_envelope_rejects_large_payloads() {
    ...
    #[test]
    fn native_api_has_request_concurrency_limit() {
