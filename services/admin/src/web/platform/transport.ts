...
function redactedResponse(result: TauriResponse): Response {
  ...
  // an accidentally supplied body instead of letting the renderer receive an
  // invalid Response or a protocol-smuggling payload.
  ...
  if (!isTauriRuntime()) {
    return (async () => {
      ...
      if (
        ...
        // Read a clone so the original Request remains available to browser
        // fetch. This keeps the fallback subject to the same byte limit as
        // the native IPC path without silently accepting an uninspected body.
