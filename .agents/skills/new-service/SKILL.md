...
2. Rename every Web and native identity: package, D1, hyphenated Worker name, dev/E2E ports, Tauri `productName` and `identifier`, Rust crate/library, fixed `TAURI_<SERVICE>_API_ORIGIN` build variable and release allowlist, navigation guard, native/browser storage keys, capability description, and Android/iOS/macOS overlays. Copy and rename `tauri-boundary.json`; explicitly review its `releaseOrigin` and every browser storage `path`, `tokenKey`, optional `organizationKey` / `logoutIntentKey`, and service-specific `reason`. Do not leave template keys/reasons, invent an implicit empty allowlist, or broaden the release-origin allowlist. Keep Vite `strictPort: true`; `TAURI_DEV_HOST` may affect development HMR only.
...
4. **Local wiring:** add the service to `DEV_ALL_SERVICES`; retain `dev` and a regular `.dev.vars.example`; run `make dev-vars`, `make -n dev/<service-name>`, and `make -n dev/all`. `make dev-vars` discovers the new SPA from the validated catalog and distributes the admin-generated local RSA pair automatically; do not add a second hard-coded service list or copy keys by hand.
