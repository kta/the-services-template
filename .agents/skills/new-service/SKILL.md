...
6. **Platform services:** register ops monitoring/backups and add the admin production service binding only for a catalog `deployable: true` domain. The non-deployable scaffold binding stays dev-only in `services/admin/vite.config.ts`; the production binding set must exactly match all deployable domains.
